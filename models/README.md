...
| `blockdude.policy.ckpt` | Block Dude phase-2 policy/value net (expert iteration on the shipped levels) | Trained 2026-09-13→14, 889k searched+demonstrated samples, trunk `[1152,1152,1152]`. Solves **15/15 shipped levels playing GREEDILY** — one forward pass per move, no search — in 3,598 moves against the human recordings' 3,870 (shorter on 11 levels, equal on 4, longer on none; Level 11 in 826 vs 909). **It is specialised to those fifteen levels and does not generalise**: 100% on them, 9% on held-out generated boards. See `docs/prd/BLOCKDUDE_REBUILD_PRD.md` §6d–§6e. |
