...
import { parseDuelingQNet } from './fruitcake-net';

// FruitCake's half of the C#/TS checkpoint pair. The "dueling-q" layout is shared with snake, tetris
// and crazy-fruits — `snake/snake-net.spec.ts` carries the full rationale for writing the bytes by hand
...
describe('parseDuelingQNet', () => {
  it('round-trips a hand-written dueling-q checkpoint', () => {
    const net = parseDuelingQNet(duelingCkpt());
  ...
  it('keeps only the mean tensors of a noisy checkpoint', () => {
    const net = parseDuelingQNet(duelingCkpt({ noisy: true }));
  ...
  it('reads a version-1 checkpoint, which carries no noisy flag at all', () => {
    ...
    const net = parseDuelingQNet(duelingCkpt({ version: 1 }));
  ...
  it('rejects a buffer whose magic is not RLNC', () => {
    expect(() => parseDuelingQNet(duelingCkpt({ magic: 'RLNX' }))).toThrow(/RLNC/);
  });

  it('rejects a checkpoint written for a different net kind', () => {
    expect(() => parseDuelingQNet(duelingCkpt({ kind: 'mlp' }))).toThrow(/kind/);
  });

  it('throws rather than returning a short net when the buffer is truncated', () => {
    const full = duelingCkpt();
    expect(() => parseDuelingQNet(full.slice(0, full.byteLength - 8))).toThrow();
  });
});
