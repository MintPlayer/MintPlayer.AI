...
public class DqnUnitTests
{
    ...
    private sealed class ConstantRewardTruncatingEnv : RLNet.Core.Environments.IEnvironment<float[], int>
    {
        private int _steps;

        public RLNet.Core.Environments.Space<float[]> ObservationSpace { get; } = new RLNet.Core.Environments.BoxSpace(-1f, 1f, 1);
        public RLNet.Core.Environments.Space<int> ActionSpace { get; } = new RLNet.Core.Environments.DiscreteSpace(2);

        public (float[] Observation, RLNet.Core.Environments.EnvInfo Info) Reset(ulong? seed = null)
        {
            _steps = 0;
            return ([0f], RLNet.Core.Environments.EnvInfo.Empty);
        }

        public RLNet.Core.Environments.StepResult<float[]> Step(int action)
            => new([0f], 1.0, Terminated: false, Truncated: ++_steps >= 5, RLNet.Core.Environments.EnvInfo.Empty);
