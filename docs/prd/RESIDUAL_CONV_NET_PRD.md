...
**Status:** M42.1 + M42.2 ✅ SHIPPED 2026-07-13 (merged to master via PR #31) · M42.3 ⛔ **CLOSED 2026-07-15 (owner decision): strength thread stopped at "loop learns, nets don't get strong at this scale."** After the draw-collapse fix (`282c665`), two further daytime experiments exhausted the config levers — `--value-weight 0.3` (conv8: no promotion in 160 games) and `--opponent-random 0.25` on the best-known recipe (conv9: no promotion in 272 games). The non-saturating eval (`--vs-minimax`, 40 games vs depth-1 material alpha-beta) scores every net at 33–40% with 0–1 wins and −20…−30 pawns: they learn to survive (draw by ply-cap) but not to convert. Full experiment journal: `data/chess-conv-autorun-log.md`. Candidate CODE levers if reopened (in recommended order): opponent-curriculum injection (train vs the minimax target; `MaterialMinimaxPlayer` exists), WDL head / z↔q blend / aux material target, or genuine scale (bigger net, sims 256, 10×+ generations) · M42.4 🟡 steps 1+3 done (`c1c7d8e`); steps 2+4 (browser wiring) stay MOOT while no conv tier beats the MLP demo. **The browser stays on the M40 MLP tier.**
**Owner:** Pieterjan
