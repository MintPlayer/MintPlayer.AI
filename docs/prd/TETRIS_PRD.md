...
- **M54.3 — Training run.** CPU, uniform no-garbage, 400K steps first pass. **Gates (protocol B primary): net-argmax survival ≥ 100 pieces mean (≥4× random, CI-separated) · net ≥ 25% of Dellacherie's gap over random on B** ((net−random)/(della−random) ≥ 0.25) · protocol A: net ≥ 5000 mean NES score (amended 2026-08-26; ≈50 single-clears with the level multiplier — random is ~4 points). Tetris rate reported (the owner's headline metric), top-out rate watched as the stack-and-camp watchdog. *Pre-registered escalation, in order: (1) `--noisy` A/B (shipped lever, §3.7); (2) longer schedule + Net2Net growth (`--grow`, spine supports it); (3) stop-loss = ship best net honestly gate-annotated (M42/M50 precedent) — the search tier still carries the showcase.*
