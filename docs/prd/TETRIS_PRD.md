...
- **Scoring**: **Amended 2026-08-26 (owner):** NES-style **40/100/300/1200** per 1/2/3/4 lines — the owner's objective is the highest score, which means building for and clearing tetrises (30× a single). The score is the reported/eval metric; the training reward is the hybrid in §3.5, not the raw score.
...
- **M54.3 — Training run.** CPU, uniform no-garbage, 400K steps first pass. **Gates (protocol B primary): net-argmax survival ≥ 100 pieces mean (≥4× random, CI-separated) · net ≥ 25% of Dellacherie's gap over random on B** ((net−random)/(della−random) ≥ 0.25) · protocol A: net ≥ 2000 mean NES score (amended 2026-08-26; ≈50 single-clears equivalent — random is ~4 points). Tetris rate reported (the owner's headline metric), top-out rate watched as the stack-and-camp watchdog. *Pre-registered escalation, in order: (1) `--noisy` A/B (shipped lever, §3.7); (2) longer schedule + Net2Net growth (`--grow`, spine supports it); (3) stop-loss = ship best net honestly gate-annotated (M42/M50 precedent) — the search tier still carries the showcase.*
