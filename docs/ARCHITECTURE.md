...
| CrazyFruits | 448 | 112 | Match-3 (M49): 6 one-hot fruit planes + would-match plane; **hard mask = match-producing swaps only**; deadlock reshuffles in-engine (mask never all-false); single-source `.pg` engine incl. the scripted random/greedy/expectimax-1 tiers. |
| Tetris | 854 | 40 | **Afterstate macro-actions** (M54): action = rot·10+col hard drop; obs = 200 board bits + piece one-hots + **sixteen** 40-wide per-action planes (M57.1 widened the M54 Dellacherie basis from six; planes 0-5 are still that basis); **the mask CAN go all-false = top-out = terminated** (unlike every other game); full NES rules (levels, ×(level+1) scoring, gravity curve) + a rising-garbage mode (per-cell garbage identity masks); single-source `.pg` engine incl. random/Dellacherie/della-search/net/net-search tiers; trained with **dense all-action regression** toward the widened evaluator at **γ=0.995**; the resume state says **n-step 1**, not the 3 the PRD and the Lab default claim (M62.4 — the recipe survives only inside the checkpoint, which is why `LabHost` now writes `<dataDir>/invocation.txt`). **M62 adds a reachability layer:** the tap budget (DAS 6 / hypertap 5 / rolling 3 frames per shift) decides what is LEGAL, not merely attractive — `simulateReach` walks the piece down against the real stack and returns both legality and the **input timeline the browser pilot replays**, so the placement you watch is the one the AI chose. Opt-in via `setReachEnforced`, so the default path and the parity checksum are untouched. **Known gap:** the evaluator tetrises but the net does not (exact argmax 18–51% tetris-rate, distilled net ~3%), and the retrain is blocked until the training recipe is re-measured. |
| 2048 | 16 | 4 | Log-scaled tiles; action masking. |
| RushHour | 72 | 32 | Two 6×6 planes; puzzle-per-episode; BFS oracle for imitation. |
| BlockDude | 1181 | 4 | Sokoban-with-gravity (M58). **Egocentric** obs — a 21×13 window over 4 planes + an 8×5 coarse density map (walls and blocks as separate channels) + 9 scalars — so the width is CONSTANT for every board size, which is what makes a size curriculum possible: the 29×19 level 11 produces the same shaped tensor as an 8×6 training board. Single-source `.pg` engine + exact BFS oracle. Moves are IRREVERSIBLE, so a misplaced block can make a level unsolvable. |
| LunarLockout | — | robots×4 | 5×5 slide-to-collision puzzle (M58). Exhaustively solvable, so it ships an exact oracle rather than a trained net; single-source `.pg`. |
...
| [`Core/Planning/ValueGuidedSearch.cs`](../src/MintPlayer.AI.ReinforcementLearning.Core/Planning/ValueGuidedSearch.cs), [`GreedyValuePlanner.cs`](../src/MintPlayer.AI.ReinforcementLearning.Core/Planning/GreedyValuePlanner.cs) | Weighted-A\* / greedy solve over a learned cost-to-go. `SolveBatched` scores a whole round of successors in one forward — **always prefer it**; the per-node `Solve` under-feeds the matrix multiply, and moving Block Dude onto the batched path alone took it 6/15 → 7/15 on the same weights. |
| [`Core/Planning/PolicyBeamSearch.cs`](../src/MintPlayer.AI.ReinforcementLearning.Core/Planning/PolicyBeamSearch.cs) | Policy-ranked beam search — **the tier for solutions that are too DEEP for a node budget**. Costs `width × depth` instead of holding a growing frontier, so a 900-move solution is in range where no A\* budget reaches it (Block Dude: 8/15 → **10/15**, adding two levels no other tier solves). Ranked by cumulative log-π alone, which is valid because every candidate sits at the same depth. **Incomplete** — a pruned solution is gone — so it complements the A\* searches rather than replacing them. |
| [`Core/Planning/PolicyValueSearch.cs`](../src/MintPlayer.AI.ReinforcementLearning.Core/Planning/PolicyValueSearch.cs) | The same batched search using **both** heads: `f = g + weight·h + policyWeight·surprise`, where surprise is the accumulated −log π of the moves that reached a node. For nets whose policy head is better trained than the value head (imitation / expert iteration), which is the usual case. Reduces exactly to `ValueGuidedSearch.SolveBatched` at `policyWeight = 0`. **The prior biases order, it never prunes** — a hard policy mask can make a solvable problem unsolvable. |
...
| [`TrainingDirectoryLock.cs`](../src/MintPlayer.AI.ReinforcementLearning.Core/Checkpoints/TrainingDirectoryLock.cs) | Exclusive claim on a `--data` directory so two runs can never interleave their CSV and checkpoints (M57; it happened). An **OS file handle** (`FileShare.None` + `DeleteOnClose`), not a PID file, so a hard kill cannot strand the directory — which matters because a running `Lab.exe` locks build outputs and gets killed routinely. Acquired in `LabHost.Run`; read-only modes (`--eval-only`, `--baselines`) skip it. The failure message names the holding pid and its full command line. |
...
### Block Dude — two training phases, and why the second exists (M58, M59)

Read **PLAN.md M59** before changing anything here: the phase-1 net solves **0/15 shipped levels** greedily, and
the reasons are measured rather than guessed (`WEBGAMES_RETIREMENT_PRD.md` §8.4a).

| File | Role |
|---|---|
| [`BlockDudeBoard.cs`](../src/MintPlayer.AI.ReinforcementLearning.Environments/BlockDude/BlockDudeBoard.cs) | Board + `BlockDudeOracle`. The oracle runs a forward BFS then a **backward** BFS from won states, so `dist = -1` marks reachable-but-lost positions. |
| [`levels/blockdude-levels.json`](../src/MintPlayer.AI.ReinforcementLearning.Environments/BlockDude/levels/blockdude-levels.json) | The 15 shipped levels. Generated by `tools/blockdude_levels.py` — never hand-edit. |
...
| [`snake/`](../src/RLDemo.Web/ClientApp/src/app/snake/) `snake_solver.ts` (generated, gitignored) · [`snake-net.ts`](../src/RLDemo.Web/ClientApp/src/app/snake/snake-net.ts) · [`snake-director.ts`](../src/RLDemo.Web/ClientApp/src/app/snake/snake-director.ts) · [`snake-renderer.ts`](../src/RLDemo.Web/ClientApp/src/app/snake/snake-renderer.ts) · [`snake.ts`](../src/RLDemo.Web/ClientApp/src/app/snake/snake.ts) | Snake, fully client-side (PLAN M33/M34/M48): generated engine + `PgSnakeNet` + BOTH planners (edit `snake_solver.pg` and build) — `chooseActionSearch` (M34 look-ahead) and `chooseActionCycle` (M48 Hamiltonian safety cycle). Director takes a `'search' \| 'cycle'` strategy and emits `SnakeAiFrame`s. **Renderer (M35 + M60):** one uniform-width round-cornered tube stroked through interpolated cell centres, rAF-glided across the 120 ms tick and **parked at `p ≥ 1`** (a resting board costs no frames); plus the **cycle overlay** — the full loop as a 5 px `#3a4154` hairline (a `Path2D` cached on `(cycleEpoch, cell)`) with the head→food arc overdrawn in `#ffd479` at 0.45, flashing once for 300 ms whenever the plan changes. Drawn *under* the tube, which is load-bearing: the body is a contiguous run behind the head on the cycle, so the head→food arc always lies on non-body cells and is never occluded. The board sizes its backing store to its real CSS width (`tetris.ts` pattern), so drawing units are CSS px. |
...
| [`tetris/`](../src/RLDemo.Web/ClientApp/src/app/tetris/) `tetris_solver.ts` (generated, gitignored) · [`tetris-net.ts`](../src/RLDemo.Web/ClientApp/src/app/tetris/tetris-net.ts) · [`tetris-game.ts`](../src/RLDemo.Web/ClientApp/src/app/tetris/tetris-game.ts) · [`tetris-director.ts`](../src/RLDemo.Web/ClientApp/src/app/tetris/tetris-director.ts) · [`tetris-render.ts`](../src/RLDemo.Web/ClientApp/src/app/tetris/tetris-render.ts) · [`tetris.ts`](../src/RLDemo.Web/ClientApp/src/app/tetris/tetris.ts) | Tetris, fully client-side (PLAN M54): generated engine — NES rules, garbage identity masks, scripted tiers, `PgTetDuelingNet` and BOTH search tiers (edit `tetris_solver.pg` and build); `.ckpt` parser for kind `dueling-q` (byte reference: `TetrisNetParityTests` via the facade's `LoadNet`); animating host with TWO drive modes over one lock path — human micro-play (NES gravity curve, wall/floor-kick rotation, soft-drop hygiene) and watch macro-placements (cosmetic glide); director (tiers Random/Dellacherie/Della-search/Net/Net+search, stale-ckpt fallback to Dellacherie); component with keyboard + pointer input, Esc pause that hides the field, and auto-pause on window blur/tab hide. **M62:** the watch pilot no longer re-derives a route — it REPLAYS the engine's input timeline on the NES frame clock, so a piece locking short of target is a logged error rather than a silent substitution (it used to hard-drop wherever it stood after two blocked inputs, quietly playing a different placement than the one chosen). Defaults are **Dellacherie + search** and **rolling**, with reachability enforced; page controls add start level (0/9/15/18/19/29), the input technique (DAS/hypertapping/rolling) and the post-29 variant (authentic / halt-at-39 / CTWC 2×ks). CCW rotation is **Z or Shift** (Ctrl triggers Windows' magnifier). |
| [`block-dude/`](../src/RLDemo.Web/ClientApp/src/app/block-dude/) `blockdude_solver.ts` (generated, gitignored) · [`block-dude-render.ts`](../src/RLDemo.Web/ClientApp/src/app/block-dude/block-dude-render.ts) · [`block-dude.ts`](../src/RLDemo.Web/ClientApp/src/app/block-dude/block-dude.ts) | Block Dude, fully client-side (PLAN M58): generated engine **and exact BFS oracle** from `blockdude_solver.pg` — the same code that labels training data also runs here. Levels are fetched from `wwwroot/levels/blockdude-levels.json`, the very bytes the Environments assembly embeds. Renderer: fused full-bleed terrain, the carryable block as the only rounded/inset element, a five-rectangle figure whose facing is a brow notch (readable at a 22px cell, where an eye dot is sub-pixel), walk 110ms, climb up-THEN-across 150ms, falls accelerating as n·p². **M62:** the level picker sits ABOVE the board (the stage has no fixed aspect ratio — it is bound per level, 19×8 … 29×19 — so anything below it shifted on every level change), and **Enter** advances to the next level once the current one is solved. |
| [`lunar-lockout/`](../src/RLDemo.Web/ClientApp/src/app/lunar-lockout/) `lunarlockout_solver.ts` (generated, gitignored) · [`lunar-lockout-render.ts`](../src/RLDemo.Web/ClientApp/src/app/lunar-lockout/lunar-lockout-render.ts) · [`lunar-lockout.ts`](../src/RLDemo.Web/ClientApp/src/app/lunar-lockout/lunar-lockout.ts) | Lunar Lockout, fully client-side (PLAN M58): generated engine + exact oracle, so the in-page **hint is the same BFS that verified every shipped level's move count**. Rockets aim by pointer position and launch on release; an ILLEGAL direction never rotates the glyph, which is the primary teaching of "the board edge is not a backstop". Renderer exposes a narrow `hover()` entry point beside `push(snapshot)` so per-frame aim state never allocates a snapshot. |
...
| Add a game to the web | **Client-side is the default** (every game since M32): rules + inference in one `.pg`, routed to a TS twin by `pgconfig.json`, then component + route + nav + home card. No controller, no per-viewer server cost. The server-authoritative path (`…ModelService` → controller → `<game>-api.ts`) is only for models that genuinely cannot run in the browser — today Rush Hour, 2048 and Cube. See [ADDING_A_GAME.md](ADDING_A_GAME.md). |
| Ship level content | One canonical JSON under `src/…Environments/<Game>/levels/`, embedded there for training (`EmbeddedLevelPack`) and copied into `wwwroot/levels/` by the `CopyLevelPacks` target for the browser — **the same bytes on both sides**, with the `wwwroot` copy gitignored. Generate it with a committed script in `tools/`; never hand-edit. |
| Add a training campaign | `ITrainingCampaign` + a `<game>Lab` + dispatch in `Lab/Program.cs`; optional A/B + search-eval harnesses. |
| Grow a net when it saturates | `Core/Training/`: `GrowthPlateau` (running max + patience over a held-out metric), `GrowthLadder` (`FromTrunk` roots it at the net's OWN default, so growth can never start smaller), `SaturationGrowth.Observe`. Feed it a **held-out quality metric on a deterministic cadence** — never the loss, never the wall-clock `Evaluate()`. Each game needs its own ladder. |
| Add a training campaign (viewer) | Implement `INetworkTelemetrySource` or `--viz` silently does nothing. `CampaignTelemetryTests` fails any campaign that forgets. |
| Release the SDK | Bump `<RLNetVersion>` in `src/Directory.Build.props` (lockstep across the five libs). **Forgetting this fails silently:** `build-master` packs and pushes on every master build with `--skip-duplicate`, so an unchanged version publishes nothing and reports success — the code ships to the playground while the NuGet packages stay on the old version. Bump it in the PR that changes a packable lib, not afterwards. |
