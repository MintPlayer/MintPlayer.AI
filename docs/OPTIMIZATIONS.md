...
| P.2 | **Resident Adam-state checkpointing** — both resident trainers (`DeviceResidualTrainer` for the cube, `DeviceConvResidualTrainer` for chess/M44) keep Adam moments on-device; they aren't yet downloaded into the campaign's Adam checkpoint, so a resumed `--gpu` run re-warms the optimizer (net weights resume fine). Add m/v download/upload in `SyncToHost` to make the resident path's resume lossless. | M20 Stage 3 follow-up (shared cube+chess fix) | lossless resume for the resident residual + conv campaigns |
...
| F.3 | **GPU-resident batched forward for the conv net (conv analogue of `DeviceMlp`/`DeviceResidualMlp`, §3.2–3.3)** — keep the conv tower's weights on-device and run its batched forward resident (chained conv→LN→ReLU→residual→heads), re-syncing only on target/checkpoint. The chess conv net currently routes through `Backend.Current` (re-uploading per GEMM); the cube value net already has this via `DeviceMlp`. This is the one piece that would make the chess-MCTS GPU path as efficient as the cube-value path and **unify the two families' GPU inference** (see `ARCHITECTURE.md` §4 "Two distinct neural-search families", `prd/RESIDUAL_CONV_NET_PRD.md` §8). | **Parked behind the scale decision.** Only pays off once a real GPU/cluster chess run is actually happening, and is best done together with **batched leaf inference** (`Mcts.SearchBatched`, shipped `4801c98`) — resident weights + big leaf batches are what make a GPU worthwhile for MCTS self-play. A genuine build (new resident conv kernels), not a refactor; don't retrofit speculatively. **✅ BUILT** (M43, commits `852cf31`/`b49a4c2`/`f39cf2f`): `Ilgpu/DeviceConvPolicyValueNet` behind the two-headed `Core/Nn/IPolicyValueForward` seam; needed exactly **two** new kernels (device im2col + scatter/bias), the rest reuse existing resident kernels. Correctness verified on the ILGPU CPU accelerator (`IlgpuBackendTests.DeviceConvForward_matches_autograd_conv_net`) AND on an RTX 3060 Laptop GPU (`ChessLab --bench-forward`): **resident 14.9× faster** than the non-resident autograd path (109.9 vs 1634.7 ms/forward at 64f×6b, leaf-batch 256 → 2,329 vs 157 leaves/s), on-GPU parity ~1e-6. Plan: [`prd/GPU_RESIDENT_CONV_PRD.md`](prd/GPU_RESIDENT_CONV_PRD.md). |
